...
const JobDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user?.email === buyer?.email)
      return toast.error("You can not apply in this job!");

    // 1. check Deadline crossed validation
    if (compareAsc(new Date(), new Date(deadline)) === 1)
      return toast.error("Deadline Crossed, Bidding Forbidden!");
    // 2. price validation
    if (price > max_price) return toast.error("Price is more than budget");

    // 3. offered deadline is within sellers deadline validation
    if (compareAsc(new Date(startDate), new Date(deadline)) === 1)
      return toast.error("please give a valid deadline");
